...
export default class LineItem {
  ...
  addOption(group, item) {
    ...
    const canAdd = (group.max_options === 0) || (group.max_options > (groupConfig ? groupConfig.optionItems.length : 0));
  ...
  removeOption(item) {
    ...
    const firstMatchingOptionItem = find(match.optionItems, oi => oi.id === item.id);
  ...
  _applyDefaultConfiguration() {
    (this.product.option_groups || []).forEach((og) => {
      og.option_items.forEach((oi) => {
  ...
  _generateOperationMaps() {
    const maps = sortBy(map(this.optionGroups(), (optionGroup) => {
      ...
      const canAddMoreToThisGroup = (optionGroup.max_options === 0) || (currentlySelectedCount < optionGroup.max_options);
