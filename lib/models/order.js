...
import { PaymentTypes } from 'brandibble/utils';
import {
  customerValidations,
  addressValidations,
  cardValidations
...
export default class Order {
    ...
    return this.adapter.persistCurrentOrder(this).then(() => {
      switch(paymentType) {
        ...
        case PaymentTypes.CREDIT:
          ...
          //In the case that we need to set payment on order to null
          if(cardOrCashTip === null) {
  ...
  setAddress(address) {
    ...
    //In the case that we need to set address on order to null
    if(address === null) {
